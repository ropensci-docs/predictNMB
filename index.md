...
[predictNMB](https://docs.ropensci.org/predictNMB/) has two main
functions:

- [`do_nmb_sim()`](https://docs.ropensci.org/predictNMB/reference/do_nmb_sim.md):
  takes user-defined inputs for a given prediction model and population,
  then evaluates the NMB by performing simulations.
- [`screen_simulation_inputs()`](https://docs.ropensci.org/predictNMB/reference/screen_simulation_inputs.md):
  calls
  [`do_nmb_sim()`](https://docs.ropensci.org/predictNMB/reference/do_nmb_sim.md)
  many times, using a range of values for any of its inputs. This is
...
is a `predictNMB` function that makes creating the function easier! If
you create these functions without
[`get_nmb_sampler()`](https://docs.ropensci.org/predictNMB/reference/get_nmb_sampler.md),
you can’t make the cost-effectiveness plot but can still evaluate in
terms of NMB.
...
- `cl`: (Optional) users can pass a cluster made using
  ...
  If it is given, the simulations are run in parallel (faster).
...
can be used on the object returned from this function to quickly inspect
these trends:
...
0.9, we can run that simulation alone using
[`do_nmb_sim()`](https://docs.ropensci.org/predictNMB/reference/do_nmb_sim.md),
or access it from our existing screen.
...
Note that we have added some tasteful themes using
[`theme_sim()`](https://docs.ropensci.org/predictNMB/reference/theme_sim.md)
that help reduce clutter on these types of plots.
...
function, specifying the treat-none option as the reference strategy.
